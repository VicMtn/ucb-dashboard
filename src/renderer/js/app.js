    ...
    <div class="project-card" data-id="${p.id}">
      <div class="project-card-header">
        ...
        <div class="project-card-menu">
          <button class="card-menu-btn" title="Renommer" onclick="event.stopPropagation();openRenameModal('${p.id}')">✏️</button>
          <button class="card-menu-btn delete" title="Supprimer" onclick="event.stopPropagation();openDeleteModal('${p.id}')">🗑</button>
        </div>
      </div>
...
function renderProjectHeader() {
  ...
  $('proj-lot').textContent  = p.lot ? `Tableau de bord — ${p.lot}` : 'Tableau de bord';
...
function renderDashboard() {
  ...
  const pctGlobal     = budgetTotal > 0 ? (utiliseTotal / budgetTotal * 100).toFixed(1) : 0;
  ...
  const crBadges = d.cr.map(c => {
    const cls = c.statut === 'approuve' ? 'approuve' : c.statut === 'attention' ? 'attention' : 'ouvert';
    const icon = c.statut === 'approuve' ? '✔' : c.statut === 'attention' ? '⚠' : '●';
    return `<span class="cr-badge ${cls}">${icon} ${esc(c.ref)}</span>`;
  }).join('');
  ...
  const budgetRows = d.budget.map(r => {
    const pct = +r.budget_reel > 0 ? (+r.utilise / +r.budget_reel * 100).toFixed(0) : 0;
  ...
  <div class="main">
    ...
    <div class="grid-3 mb-20">
      ...
      <!-- Détail technique -->
      <div class="card">
        <div class="card-title">⚙️ Détail Avancement</div>
      ...
      <div class="card">
        ...
        <div class="donut-wrap">
          <svg width="90" height="90" viewBox="0 0 100 100">
            <circle cx="50" cy="50" r="38" fill="none" stroke="#f0f2f4" stroke-width="14"/>
            ${(()=>{
              const total = +doc.appro + +doc.revue + +doc.attente + +doc.nouvelles || 1;
              const a = +doc.appro/total*C, b=+doc.revue/total*C, c=+doc.attente/total*C, dd=+doc.nouvelles/total*C;
              return `
    ...
    <div class="grid-2 mb-20">
      ...
      <div class="card" style="display:flex;flex-direction:column;gap:16px;">
        <div class="card-title">🌐 Utilisation Globale</div>
    ...
    <div class="grid-2">
      ...
      <div class="card">
        <div class="card-title">⚠️ Risques Majeurs</div>
        <table class="budget-table">
          <thead><tr><th>Risque</th><th>Domaine</th><th>Impact</th></tr></thead>
          <tbody>${d.risques.map(r => `
            <tr>
            ...
            </tr>`).join('') || '<tr><td colspan="3" style="color:var(--gray-400);font-size:13px;">Aucun risque enregistré</td></tr>'}
          </tbody>
        </table>
      </div>
    </div>
...
function renderAvancementForms() {
  ...
  <div class="main">
    ...
    <div class="section-title">⚙️ Détail technique</div>
    <div class="card mb-20">
      <div class="form-row">
  ...
  $('view-avancement').querySelectorAll('[data-bind]').forEach(el => {
...
function renderBudgetView() {
  ...
  <div class="main">
    ...
    <div class="card mb-20">
      ...
      <button class="add-btn" id="budget-add-btn">+ Ajouter une catégorie</button>
    </div>
    ...
    <div class="card">
      ...
      <button class="add-btn" id="cr-add-btn">+ Ajouter un CR</button>
    </div>
  </div>`;
...
function renderBudgetRows() {
  $('budget-tbody').innerHTML = state.data.budget.map((r,i) => `
    <tr>
      ...
      <td><button class="del-btn" onclick="deleteBudget(${i})">✕</button></td>
    </tr>`).join('');
}
...
function renderCRRows() {
  $('cr-tbody').innerHTML = state.data.cr.map((r,i) => `
    <tr>
      ...
      <td><button class="del-btn" onclick="deleteCR(${i})">✕</button></td>
    </tr>`).join('');
}
...
function renderEtapesView() {
  ...
  <div class="main">
    ...
    <div class="card">
      ...
      <button class="add-btn" id="etapes-add-btn">+ Ajouter une étape</button>
    </div>
  </div>`;
  renderEtapesRows();
...
function renderEtapesRows() {
  $('etapes-tbody').innerHTML = state.data.etapes.map((r,i) => `
    <tr>
      ...
      <td><button class="del-btn" onclick="deleteEtape(${i})">✕</button></td>
    </tr>`).join('');
}
...
function renderRisquesView() {
  ...
  <div class="main">
    <div class="section-title">⚠️ Risques majeurs</div>
    <div class="card">
      <table class="editable-table">
        <thead><tr><th>Risque</th><th>Domaine</th><th>Impact</th><th></th></tr></thead>
        <tbody id="risques-tbody"></tbody>
      </table>
      <button class="add-btn" id="risques-add-btn">+ Ajouter un risque</button>
    </div>
  </div>`;
  renderRisquesRows();
...
function renderRisquesRows() {
  $('risques-tbody').innerHTML = state.data.risques.map((r,i) => `
    <tr>
      ...
      <td>
        <select class="form-select" onchange="updateRisque(${i},'domaine',this.value)">
          ${['Technique','Fournisseur','Financier','Planning','Autre'].map(d =>
            `<option value="${d}" ${r.domaine===d?'selected':''}>${d}</option>`).join('')}
        </select>
      </td>
      <td><input class="form-input" value="${esc(r.impact)}" onchange="updateRisque(${i},'impact',this.value)"></td>
      <td><button class="del-btn" onclick="deleteRisque(${i})">✕</button></td>
    </tr>`).join('');
}
...
function openDeleteModal(id) {
  const p = state.projects.find(x => x.id === id);
  _deleteId = id;
  $('modal-delete-text').textContent = `Supprimer "${p?.name}" ? Cette action est irréversible, toutes les données seront perdues.`;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('modal-proj-name').addEventListener('keydown', e => { if (e.key === 'Enter') confirmProjectModal(); });

  // Modal delete
  $('modal-delete-cancel').onclick  = () => $('modal-delete').classList.add('hidden');
